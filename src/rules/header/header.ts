...
const SEVERAL_H1 = (node: AstObject, state?: any): void | object => {
  const { children } = node;
...
const INVALID_H2_POSITION = (node: AstObject, state?: any): void | object => {
...
const INVALID_H3_POSITION = (node: AstObject, state?: any): void | object => {
  if (!state.h2) {
    return;
  }
